...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "accuracy_benchmark_200_combinatorial"
BATCH_SIZE = 10
...
def brute_expected(index):
    ...
    return {
        ...
        "s": s,
        "limit": limit,
        "expected": sorted(values, key=lambda item: tuple(int(part) for part in item.strip("()").split(","))),
    }
...
def make_tasks():
    ...
    index = 0
    while len(tasks) < 200:
        data = brute_expected(index)
        index += 1
        if not (3 <= len(data["expected"]) <= 18):
            continue
        tasks.append(
            {
                "id": task_id,
                "problem": (
                    f"求所有整数有序数对 (x,y)，满足 -{data['bound']}<=x,y<={data['bound']}，"
                    f"x^2+3y ≡ {data['r']} (mod {data['m']})，"
                    f"2x-y ≡ {data['s']} (mod {data['n']})，"
                    f"|x|+|y| <= {data['limit']}。必须列出全部有序对。"
                ),
...
def run_deepseek_batch(tasks):
    system_prompt = (
        ...
        "不要用省略号，不要给步骤，不要遗漏负数解。"
    )
    user_content = json.dumps(
        {
            "instruction": "解这些有界整数同余条件题。只输出 JSON：{\"answers\":[{\"id\":1,\"solutions\":[\"(x,y)\"]}]}",
    ...
    return call_deepseek_json(
        ...
        max_tokens=10000,
        agent_name="deepseek_chat_200_combinatorial_accuracy",
...
def draw_chart(summary, output_path):
    ...
    draw.text((50, 34), "高难度组合枚举准确率对比", fill="#111827", font=title_font)
    draw.text((50, 82), "有界整数解、同余条件、绝对值约束", fill="#4b5563", font=small_font)
...
def main():
    ...
    (OUTPUT_DIR / "combinatorial_200_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    lead = round(summary["alleras"]["accuracy"] - summary["deepseek"]["accuracy"], 1)
    (OUTPUT_DIR / "combinatorial_200_report.md").write_text(
        "\n".join(
            [
                "# 高难度组合枚举准确率对比",
                "",
    ...
    draw_chart(summary, OUTPUT_DIR / "combinatorial_200_bar.png")
